...
required_node_version="12.14.0"
required_npm_version="6.13.4"
...
upgrade_node()
{
    ...
    case $os in
        ...
        "Darwin")
            ...
            curl -k -O https://nodejs.org/dist/v$required_node_version/node-v$required_node_version-darwin-x64.tar.gz
            tar -xzf ./node-v$required_node_version-linux-x64.tar.gz -C $node_path --strip-components=1 --no-same-owner > /dev/null 2>&1
            rm -f ./node-v$required_node_version-linux-x64.tar.gz > /dev/null 2>&1
...
if [[ "$arch" == "armv6l" ]]; then
    echo "Can Not Install Node"
    echo "------------------------------------------------------------"
    echo "Node $required_node_version is not supported on your system."
    echo "------------------------------------------------------------"
